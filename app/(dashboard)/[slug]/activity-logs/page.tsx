...
export default async function ActivityLogsPage(props: { searchParams: Promise<any> }) {
  ...
  const response = await getActivityLogsAction(page, limit);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl border border-gray-100 shadow-xl">
            ...
            <tbody className="divide-y divide-gray-100">
              {logs.map((log: any) => (
                <tr key={log._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-sm text-gray-600 max-w-xs truncate" title={log.details}>
                    {log.details || "-"}
                  </td>
                </tr>
              ))}
