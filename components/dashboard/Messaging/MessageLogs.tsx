...
export default function MessageLogs({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const getStatusBadge = (log: LogEntry) => {
    const status = log.lastStatus || "PENDING";
    const errorDetails = log.lastErrorDetails;
  ...
  return (
    <>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
                <TableHead className="w-[180px]">Last Message</TableHead>
      ...
      {/* Chat Panel — always opened by patient identity, so it shows that
          patient's complete message history (see PatientChatPanel's
          individual-mode fetch), not a single campaign/batch. */}
      <PatientChatPanel
        isOpen={chatOpen}
