...
const typeBadge: Record<string, string> = {
  ...
  BOOKING_CONFIRMATION: "bg-sky-100 text-sky-600",
...
function StatusTick({
  ...
  switch (status) {
    ...
    default:
      return (
        <Clock className="w-3.5 h-3.5 text-slate-300 animate-pulse shrink-0" />
      );
  }
}
...
function RecipientStatusPill({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    DELIVERED: "bg-violet-50 text-violet-600 border-violet-100",
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${map[status] ?? "bg-slate-50 text-slate-500 border-slate-200"}`}
    >
...
export default function PatientChatPanel({
  ...
  const fetchHistory = async () => {
    ...
    try {
      let url = "";
      if (isGroup && batchId)
        url = `/api/whatsapp/chat?batchId=${encodeURIComponent(batchId)}`;
      else if (isGroup && groupKey)
        url = `/api/whatsapp/chat?groupKey=${encodeURIComponent(groupKey)}`;
      else if (patientId) url = `/api/whatsapp/chat?patientId=${patientId}`;
      else if (phone)
        url = `/api/whatsapp/chat?phone=${encodeURIComponent(phone)}`;
      if (!url) return;
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      await axios.post("/api/whatsapp/manual-send", {
        patientId,
        phone,
        message: msg,
      });
  ...
  const broadcastDate = messages[0]
    ? formatDate(messages[0].sentAt ?? messages[0].createdAt)
    : "";
  const broadcastTime = messages[0]
    ? formatTime(messages[0].sentAt ?? messages[0].createdAt)
    : "";
  ...
  return (
    <>
      ...
      >
        {/* ── header (kept WhatsApp green intentionally) ── */}
          ...
          <div className="flex-1 min-w-0">
            ...
            {isGroup ? (
              <p className="text-xs text-green-100 mt-0.5">
                {sentCount} sent ·{" "}
                {failedCount > 0 ? `${failedCount} failed · ` : ""}Campaign
                broadcast
              </p>
            ) : (
        ...
        {/* ── chat area (kept WhatsApp wallpaper + bubble green intentionally) ── */}
        <div
          className="flex-1 overflow-y-auto p-4"
          style={{
            background: "#efeae2",
            backgroundImage:
              "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23d5ccbb' fill-opacity='0.4'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")",
          }}
        >
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end">
                <div className="max-w-[88%] bg-[#d9fdd3] rounded-xl rounded-tr-sm px-3 py-2.5 shadow-sm">
                  <span
                    className={`inline-block text-[9px] font-bold uppercase px-1.5 py-0.5 rounded mb-1.5 ${typeBadge[broadcastType] ?? "bg-slate-100 text-slate-500"}`}
                  >
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div className="divide-y divide-slate-50">
                  {messages.map((msg) => {
                    const p =
                      typeof msg.patientId === "object" ? msg.patientId : null;
                    const name = p
                    ...
                    return (
                      ...
                      >
                        <div className="w-8 h-8 rounded-full bg-[#0D1117] flex items-center justify-center text-emerald-400 text-xs font-bold shrink-0">
              ...
              <div className="flex justify-center">
                <span className="bg-amber-50 border border-amber-100 text-amber-700 text-[11px] px-3 py-1.5 rounded-lg text-center max-w-xs">
                  Broadcast campaign · To reply to a specific patient, open
                  their profile.
                </span>
              </div>
            </div>
          ) : (
            /* ── INDIVIDUAL MODE (kept WhatsApp bubble green intentionally) ── */
                  ...
                  <div key={msg._id} className="flex justify-end mb-1.5">
                    <div className="max-w-[82%] bg-[#d9fdd3] rounded-xl rounded-tr-sm px-3 py-2 shadow-sm">
                      {msg.messageType !== "MANUAL" && (
                        <span
                          className={`inline-block text-[9px] font-bold uppercase px-1.5 py-0.5 rounded mb-1 ${typeBadge[msg.messageType] ?? "bg-slate-100 text-slate-500"}`}
                        >
        ...
        {/* ── input (kept WhatsApp send-button green intentionally) ── */}
        {!isGroup && !readOnly && (
          <>
            <div className="shrink-0 px-3 py-3 bg-[#f0f2f5] border-t border-slate-200 flex items-end gap-2">
              <Textarea
                ...
                className="flex-1 min-h-[44px] max-h-[120px] resize-none rounded-2xl border-0 bg-white shadow-sm text-sm focus-visible:ring-1 focus-visible:ring-emerald-300 px-4 py-3"
                rows={1}
              />
              <Button
                onClick={handleSend}
            ...
            <div className="shrink-0 px-4 py-2 bg-amber-50 border-t border-amber-100 flex items-center gap-2">
              ...
              <p className="text-[11px] text-amber-700">
                Free-text works within 24 hrs of patient's last reply. Otherwise
                use a template.
              </p>
            </div>
          </>
