...
export function Header() {
  ...
  const getPageTitle = (path: string) => {
    ...
    return (
      activePath.charAt(0).toUpperCase() + activePath.split("/")[0].slice(1)
    );
  };
  ...
  return (
    <header className="shrink-0 overflow-hidden sticky top-0 z-10 border-b border-slate-200 bg-white h-16 px-4 md:px-6 flex items-center justify-between shadow-sm">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <h2 className="text-sm sm:text-lg font-bold text-slate-900 tracking-tight leading-tight truncate max-w-40 sm:max-w-xs md:max-w-md">
            {title}
          </h2>
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {clinic?.plan && (
              <span
                className={`px-2 py-0.5 inline-flex text-[9px] leading-4 font-bold rounded-full uppercase tracking-wider shadow-sm ${
      ...
      <div className="flex items-center gap-3 sm:gap-5">
        {/* Date Badge (Desktop) */}
        <div className="hidden md:flex items-center gap-2 bg-emerald-50 px-4 py-2 rounded-full border border-emerald-100 shadow-sm hover:border-emerald-200 transition-all duration-200 group">
        ...
        <div className="flex items-center gap-3 pl-1">
          ...
          >
            <div className="w-full h-full rounded-full bg-[#0D1117] flex items-center justify-center text-emerald-400 text-xs font-bold">
